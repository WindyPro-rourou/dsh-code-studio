...
import { watch as fsWatch, statSync, realpathSync, readFileSync } from "node:fs";
...
class FileLedger {
   ...
   */
  captureRevertPoint(path, sessionId) {
    if (typeof path !== "string" || path === "" || typeof sessionId !== "string") return;
    ...
    try {
      // Synchronous read: tool/call fires before the agent's write, so this
      // snapshots the true pre-write content even for instant writes.
      const st = statSync(path);
      existed = true;
      mtimeMs = st.mtimeMs;
      size = st.size;
      if (st.size <= MAX_READ_BYTES) content = readFileSync(path, "utf8");
    } catch (error) {
      if (error.code !== "ENOENT") return;
    }
    // Seed the baseline with the pre-write content: without this, the first
    // handleChange after the agent's write would diff against nothing and the
    // panel would show the whole file as "new" lines.
    if (existed && content !== null) this.recordBaseline(path, content, mtimeMs, size);
    ...
    try {
      ...
      const addSessionRoot = (session) => {
        if (session?.header?.cwd) {
          ledger.addRoot(session.header.cwd);
          ledger.noteSessionRoot(session.header.cwd, session.id);
        }
      };
      try {
      ...
      const offSession = ctx.on("session/event", (session, event) => {
        ...
        if (event.type === "tool/call") {
          // DSH >= 0.1.1-rc.2 serializes tool arguments as a JSON string on the
          // wire; older versions passed the object directly. Accept both.
          if (typeof data.name === "string") {
            let args = data.arguments;
            if (typeof args === "string") {
              try { args = JSON.parse(args); } catch { args = void 0; }
            }
            if (args && typeof args === "object") {
              ...
              if (PATH_TOOLS.has(data.name)) {
                const abs = toolPath(args, session?.header?.cwd);
